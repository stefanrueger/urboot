/*
 * Pins definitions for ATmega16/32
 * Changed from Felix Rusu's pins_arduino.h for the ATmega1284P (which is close to the older model)
 * the might be errors owing to that - Stefan Rueger
 */
...
#define digitalPinToInterrupt(p) ((p) == 10? 0: (p) == 11? 1: NOT_AN_INTERRUPT)
