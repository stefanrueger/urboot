 ...
 *   atmega128rfr2 atmega16 atmega161 atmega162 atmega163 atmega164a atmega164p atmega164pa
 *   atmega165 atmega165a atmega165p atmega165pa atmega168 atmega168a atmega168p atmega168pa
 *   atmega168pb atmega169 atmega169a atmega169p atmega169pa atmega16a atmega16hva atmega16hva2
 *   atmega16hvb atmega16hvbrevb atmega16m1 atmega16u2 atmega16u4 atmega2561 atmega2564rfr2
 *   atmega256rfr2 atmega32 atmega323 atmega324a atmega324p atmega324pa atmega325 atmega3250
 ...
 * didn't necessarily fit into 512 bytes. Now most features fit into the typical 512 bytes
 * bootloader of smaller MCUs and certainly into the 1k minimum boot section of bigger MCUs.
 *
 * This bootloader has many a configuration option for avr-gcc:
 *
 * -mmcu=<mcu> -DF_CPU=<num>L
 *
 * These are the two most important avr-gcc option; they select the MCU and the CPU frequency of
 * the destination board in Hertz as a long constant number, eg, -DF_CPU=16000000L. The bootloader
 * code needs to know the CPU frequency as the serial communication baud rate is derived from it.
 *
 * With the notable exception of -DSTART and -DRJMPWP (which are automatically set by the avr-gcc
 * wrapper urboot-gcc) almost all options to the compiler can be put on the command line for make
 * without the -D. So, you can create a bootloader by issuing, for example the command
 *
 * $ make MCU=attiny45 F_CPU=123456L SWIO=1 RX=AtmelPB0 TX=AtmelPB1 MOVETO=mysupderduperboard
 *
 * Unless an option can only be issued to avr-gcc I will leave the leading -D away.
 *
 *
 * -DSTART=<address>
 *
 ...
 * applications in vector bootloaders (see below) and for copying the right amount of external
 * flash memory to microprocessor flash in dual boot. The -DSTART=0x...UL option informs urboot.c
 * about the byte address where the linker will be told to put the bootloader. Coincidently,
 * FLASHEND+1UL-START is the size of the bootloader. START is actually also the address of the main
 * function. The location of the bootloader is not known at compile-time, so when using the symbol
 * main instead of the START compile-time constant the code would get larger without need. The neat
 * little accompanying perl script urboot-gcc can be used as avr-gcc wrapper that knows a lot of
 * MCUs, can compute the "right" START and auto-magically runs the avr-gcc appropriately.
 *
 *
 * EEPROM=<1|0>
 *
 * With EEPROM=1, avrdude's option -U eeprom:r:file:i saves the eeprom contents of the MCU onto a
 * file and -U eeprom:w:file:i will write the file contents onto the MCU's eeprom. It's a useful
 * property to be able to manage non-volatile data and programs independently.
 *
 *
 * VBL=<0|1|2|3> [VBL_VECT_NUM=<num>]
 *
 * The AVR interrupt vector table consists of 4-byte jmp instructions or 2-byte rjmp instructions.
 * If the MCU has no more than 8 kB flash it is always a rjmp. The reset vector is the first in the
 * table, and contains a jump instruction to the start of the application. A vector bootloader
 * (VBL=1, 3, or 3) is a hook between the reset vector and the application, where the reset vector
 * has been made to point to the bootloader, and the bootloader - once finished - jumps to a
 * dedicated but otherwise unused interrupt vector that has been made to point to the application.
 * This trick provides a bootloader on MCUs without boot section support (Trinket, Digispark,
 * ATtinyXX, ATmega48, ...). On MCUs with boot section support you can utilise a vector bootloader,
 * too, by changing the fuses to jump to 0x0000 on reset. This way the boot section can start at
 * any multiple of SPM_PAGESIZE in high flash space, which can considerably reduce the space taken
 * by the bootloader. For example, a ATmega328p system can then use 384 bytes (3 pages) rather than
 * the otherwise minimum 512 bytes boot section. Likewise, those MCUs with 1 kB minimum boot size
 * (ATmega1284p, eg) might get away with only using 512 or 768 bytes for the bootloader.
 *
 * By default, a vector bootloader loses one interrupt vector that no program ever can use on that
 * device. Urboot chooses a "rare" vector of your target MCU to store a jump instruction to the
 ...
 *
 * If reserving an interrupt vector for the vector bootloader is out of question, there is also the
 * option of enlarging the processor's interrupt vector table by one additional slot. This can be
 * done in the Arduino IDE through adding the following to the end of the hardware/arduino/avr/
 * cores/arduino/main.cpp source that will then automatically be included into every application.
 *
 ...
 * manipulates the destination address in the missing upper half (for other MCUs it will be
 * overwritten at burn time). This increases the size of each sketch by 4 or 2 bytes, respectively.
 *
 * Compiling the bootloader with the option VBL_VECT_NUM=-1 tells it to use this additional vector
 * (which, eg, would translate to 26 for the ATmega328P, 35 for the ATmega1284P, 57 for the
 * ATmega2560, 15 for the ATtiny85 and 20 for the ATtiny167).
 *
 ...
 * about this bootloader, which encodes its version number and main capabilities in the .version
 * section (see below), so that avrdude -c urclock can and will apply the patch for upload and
 * verification auto-magically. If neither avrdude nor external patching of applications fit your
 * workflow, you can alternatively compile a bootloader with VBL=2 or 3, so the bootloader does
 * this internally at the cost of enlarged bootloader size. VBL=1 is recommended as the bootloader
 * code here to patch the incoming sketch on the embedded device makes more assumptions than
 * avrdude's urclock programmer.
 *
 * Options VBL=2 or 3 both create a vector bootloader that copies the reset vector of a program
 * during upload into an unused interrupt vector and replaces the reset vector with a jump to the
 * bootloader. Once the bootloader has modified the uploaded program, avrdude -c arduino would
 * notice the changes in the verification step and complain. VBL=3 creates code that specifically
 * alters verification of the first page at address zero, which contains the vector table, so a
 * copy of the uploaded vector page is returned. This way, everything except the page with the
 * modified vector table is being verified. A serial communication error on the first page would
 * still be discovered as the bootloader returns what it has read. Note that verification of the
 * vector page is altered only when immediately done after programming, ie, when using avrdude's
 * verify option. If the program flash is read in a separate avrdude call then the real contents is
 * returned as it should be. From version 7.5 onwards the VBL=2 or 3 code checks whether the reset
 * vector already points to the bootloader, in which case the patch is not executed. Consequently,
 * verification of the page at zero is not altered. This is done to allow both un-patched and
 * patched applications be uploaded with a vector bootloader when VBL is set to 2 or 3.
 *
 * Burning a VBL-enabled urboot onto an MCU should in theory also require the jump to boot section
 * be burned into the reset vector. The accompanying urloader sketch does that.  However, one will
 * get away with not burning the reset jump address into the vector table, if the MCU is erased
 * otherwise: erased words read 0xffff, and although this is not an official opcode, it behaves as
 * sbrs r31,7 (skip one instruction if bit 7 in R31 is set). Any reset to 0x0000 on an otherwise
 * erased flash will therefore eventually run into the start of the bootloader.
 *
 *
 * DUAL=<0|1> [SFMCS=<AtmelPxx|ArduinoPinX> | TEMPLATE=1] [DUAL_NO_SECTOR_ERASE] [DUAL_NO_SPI_RESET]
 *
 * DUAL=1: When the boot sector is entered through a Watchdog timer (WDT) reset, a dual-boot
 * enabled urboot reads the first page of an attached external SPI flash memory. If that is
 * compatible with a jmp/rjmp vector table, it is assumed that the SPI flash memory contains a new
 * program to be burned onto the MCU. As there is no size associated with the copy of the program
 * the maximum number of flash pages is being copied onto the MCU right up to START, where the
 * bootloader sits. It then runs the new application. This is a crucial stepping stone for "over
 * the air" programming: The application program, when run normally, recognises "new program"
 * packets that it gets from somewhere (radio, WiFi, magic, ...) and drops them onto the start of
 * external memory. When finished, the application triggers a watchdog reset, urboot burns the new
 * program, and the application has just reprogrammed itself: Voila! Useful for those wireless
 * projects that are installed where they are kind of hard to get to.
 *
 * Unless DUAL_NO_SECTOR_ERASE is defined, code is generated within the bootloader to erase the
 * first sector of external memory directly after burning the new program. The SPI flash memory
 * code SE4k is used, which erases the first 4k sector on most SPI flash memories, followed by 1 s
 * watchdog timeout and reset. The rather long timeout was chosen as a code-space saving way to let
 * the flash memory finish the sector erasure. (Polling the SPI flash memory in the bootloader
 * seemed like a waste of code for the rare self-programming events).
 *
 * SPI will always be switched off before starting the application. Unless DUAL_NO_SPI_RESET is
 * defined the I/O ports for SPI lines are reset.
 *
 * It is *not* recommended to either define DUAL_NO_SECTOR_ERASE or DUAL_NO_SPI_RESET. This should
 * only be used if one is *really* desperate for code space.
 *
 * SFMCS=<AtmelPxx|ArduinoPinX>
 *
 * This option encodes the pin number of the SPI flash memory chip select line: either use a
 * constant like AtmelPB0 (port B, pin 0) or, eg, ArduninoPin8.
 *
 * TEMPLATE=1
 *
 * Creates a template bootloader with different nop opcodes (mov rN,rN) where the SFMCS pin needs
 * to be manipulated. These template bootloaders cannot be used as they are, but if you use the
 * urloader sketch to burn a template bootloader you can select the CS pin of your flash memory,
 * and it will put in the right sbi/cbi opcodes into the right places when burning the bootloader.
 *
 *
 * PROTECTME=<1|0> (protect bootloader from overwriting itself)
 *
 ...
 *
 * PGMWRITEPAGE=1 makes available to applications a bootloader function
 *
 *   void pgm_write_page(void *sram, progmem_t pgm);
 *
 * This burns SPM_PAGESIZE bytes from SRAM memory into PROGMEM at location pgm. When the device has
 * more than 64 kB flash (and hence RAMPZ exists) then progmem_t is a type that holds 32 bit
 * addresses, else progmem_t is a 16 bit address (void *). The caller has to take care that pgm is
 * aligned with memory pages of SPM_PAGESIZE bytes. Sram is always 16-bit. Normally, application
 * code cannot write PROGMEM, as this has to be done from the boot section, so this option provides
 * applications with the opportunity to treat unused PROGMEM a bit like EEPROM. There are library
 * functions urstoreWrite()/Read() that make use of this interface. As exporting this function
 * comes a small cost of some 4-6 bytes overhead to call the anyway needed writebuffer() function,
 * the recommendation is to always  set this option. From a security point of view, there are
 * certain risks involved, as this option allows the application to modify itself. If you are
 * concerned about this, I recommend using SPI programming instead of using any bootloader at all
 * (rendering urboot moot) and/or setting the fuses very restrictively (which renders this option
 * obsolete).
 *
 * The option -DRJMPWP=0xCxxx sets "rjmp pgm_write_page" opcode, which will be stored just below
 * the two version bytes, so that this function can be called at the address FLASHEND+1UL-4. The
 * accompanying perl compiler wrapper urboot-gcc computes RJMPWP if and only if the pgm_write_page
 * and main symbols both exist in the .elf file, see Makefile. In case that PGMWRITEPAGE=0 or that
 * the -DRJMPWP=... opcode hasn't been supplied to urboot.c then the ret opcode will be stored
 * instead of the rjmp, so that calling this interface returns empty-handed. Use as follows in your
 * application:
 *
 *   ((void (*)(void *, progmem_t))((FLASHEND+1UL-4UL)/2))(sram, pgm);
 *
 * Division by 2 reflects the fact that avr-gcc has PC word addresses. progmem_t needs to be a 32
 * bit type if your MCU has more that 64k flash, a 16 bit type otherwise. Here is a full definition
 * of the pgm_write_page() function to be used in the application sketch (cut and paste, or better,
 * use the ur/libraries/urboot library):
 *
--- snip --- snip --- snip ---
 ...
 * issues a couple of requests to set the device that are not needed here and wants to read fuses.
 * With URPROTOCOL=0 the generated code gracefully accepts these requests but ignores them. For
 * example, the bootloader pretends the fuses are 0xff.
 *
 * URPROTOCOL=1 works with a version of avrdude that offers an urclock programmer. The protocol is
 * different to STK500v1, so that an MCU id and a few other features of this bootloader are sent to
 * avrdude. Generally URPROTOCOL=1 requires the bootloader to do much less work and shifts some of
 * the work to the programmer. In this case, it is needed to run avrdude -c urclock for programming
 * and -c arduino will not work. Setting URPROTOCOL=1 saves between 76 and 112 bytes.
 *
 *
 ...
 * CHIP_ERASE=<0|1>
 *
 * With this option set to 1, an STK_CHIP_ERASE command will erase all flash below the bootloader;
 * otherwise, STK_CHIP_ERASE commands are ignored. If the URPROTOCOL option is set then this
 * bootloader will inform avrdude -c urclock whether or not it has the ability to perform a chip
 * erase, so that avrdude can emulate a chip erase if needed by erasing unset bytes in the input
 * file to 0xff. Avrdude's chip erase emulation is much slower than the bootloader's chip erase, as
 * it carries the overhead of sending 0xff bytes over the serial interface. Setting this option
 * will cost around 18-38 bytes code.
 *
 ...
 *
 * BLINK=<0|1> [LED=<xx> [LEDPOLARITY=<1|-1>] | TEMPLATE=1]
 *
 * Light effects: BLINK=1 toggles the activity LED during serial data I/O. The Option LED=xx lets
 * you set the LED pin in terms of Atmel port and bit number as in, eg, AtmelPB1 or ArduinoPin9.
 * LEDPOLARITY specifies whether the LED is assumed to be low active (LEDPOLARITY=-1) or high
 * active otherwise (LEDPOLARITY undefined or set to 1). Using TEMPLATE=1 is an alternative
 * to specifying the LED pin at compile time. This creates a template bootloader with different nop
 * opcodes (mov rN,rN) instead of code that operates the LED. If you then use the urloader sketch
 * to burn a template bootloader you can select/change the LED pin and polarity, and it will put in
 * the right sbi/cbi codes into the right places when burning the bootloader.
 *
 ...
 * FRILLS=<level>
 *
 * Shortcut for setting defaults for some of the options; default level is 0
 *
 *  - CHIP_ERASE       (FRILLS >= 7)
 *  - RETSWVERS        (FRILLS >= 6)
 *  - QEXITEND         (FRILLS >= 5)
 ...
 * programmer before starting the application. Possible values are: 16MS, 32MS, 64MS, 125MS, 250MS,
 * 500MS, 1S and 2S. Most MCUs also provide 4S and 8S. When the device has a reset circuit that can
 * be triggered into reset by pulling the DTR line of the associated FTDI chip then small times
 * such as 500MS (the default) are useful. If there is no such reset circuit and upload involves
 * manual synchronisation then higher values such as 2S are recommended. Very small values up to
 * 125MS are unlikely to provide enough time for a handshake with the uploading program.
 *
 *
 * DEBUG_FREQ=<n> FREQ_PIN=...
 *
 * On starting the bootloader swing a square wave of 5 periods on pin FREQ_PIN (can be LED) with a
 * frequency of DEBUG_FREQ Hz. This is useful of one wants to measure the frequency F_CPU of the
 * MCU on the FREQ_PIN (using another arduino or a scope). Not meant for production, only for
 * debugging. 50 Hz is a good frequency as this delays bootloader comms by only 100 ms. Any lower
 * frequency might be too much delay for synchronisation with avrdude.
 *
 *
 * FLASHWRAPS=<1|0>
 *
 * Informs urboot.c whether or not flash memory wraps around. When the bootloader finally starts
 * the application it will jump to a location in the vector table (either the reset vector or, if
 * it is a vector bootloader, to the dedicated vector). If flash memory wraps around, then the
 * bootloader can execute a short 2-byte rjmp over the end of flash to the vector table rather than
 * a absolute jmp thus saving 2 bytes of code. The default is FLASHWRAPS=1 if and only if the size
 * of flash memory is a power of 2, but can be overwritten should this assumption be wrong for a
 * MCU. This option has no effect in MCUs with up to 8k flash memory as they always use rjmp.
 *
 *
