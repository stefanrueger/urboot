...
[urprotocol](urprotocol.md)**
 ...
 - Highly modular feature set:
     ...
     + [Template](docs/makeoptions.md#template_sfm) bootloader with nops that can be replaced with
       opcodes to set/clear the right [LED](docs/makeoptions.md#template_led) or [CS
       pins](docs/makeoptions.md#template_sfm)
...
[optiboot](https://github.com/Optiboot/optiboot) with the same functionality, see
[below](README.md#comparison).

<p id="background"></p>

**Background.** More information on how bootloaders work with tips, tricks and limitations
[here](docs/background.md).

<p id="autogenerated"></p>

**Autogenerated bootloaders.** The easiest way to [generate and burn](docs/autogenerated.md) an
urboot bootloader is with AVRDUDE v8.1 or later using virtual
[`urboot:<features>`](docs/autogenerated.md#possible-features) filenames for avrdude. If that is
not an option, the sections below detail how to compile a bootloader from scratch or select one
from a repository of pre-compiled bootloaders.

<p id="how-to-select"></p>

**How to select a bootloader.** Tips, tricks and some background [here](docs/howtoselect.md).
...
**Pre-compiled bootloaders.** If compiling bootloaders is not feasible, feel free to try out one
of the 1,283,806 *different* pre-compiled bootloaders in the directory tree for [`MCU
...
The naming convention for the `.hex` files is given in the [larger comparison table covering a
range of MCUs](docs/comparison.md).
